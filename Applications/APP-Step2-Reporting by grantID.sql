-- =========================================================
-- APPLICATIONS rollups + emission factor calcs
-- - Auto-detects latest: pa_ghg_reporting.ghg_report_2025_applications_YYYYMMDD
-- - Creates:
--     pa_ghg_reporting.first_ghg_report_2025_applications_with_unespecied
--     pa_ghg_reporting.second_rewetted_summary_table_<timestamp>
--     pa_ghg_reporting.third_ghg_report_2025_summary_<timestamp>
--     pa_ghg_reporting.emission_factors_application_calcs_<timestamp>
-- - Updates views (drop + recreate to avoid type-change errors):
--     pa_ghg_reporting.second_rewetted_summary_table_latest
--     pa_ghg_reporting.third_ghg_report_2025_summary_latest
--     pa_ghg_reporting.emission_factors_application_calcs_latest
-- =========================================================
...
DECLARE
  ...
  ef_tbl     text := format('emission_factors_application_calcs_%s', ts);
BEGIN
  --------------------------------------------------------------------
  -- Find latest applications output table
  --------------------------------------------------------------------
  SELECT format('%I.%I', schemaname, tablename)
  INTO v_src
  FROM pg_tables
  WHERE schemaname = 'pa_ghg_reporting'
    AND tablename LIKE 'ghg_report_2025_applications_%'
  ORDER BY tablename DESC
  LIMIT 1;

  IF v_src IS NULL THEN
    RAISE EXCEPTION 'No table found matching pa_ghg_reporting.ghg_report_2025_applications_YYYYMMDD';
  END IF;

  RAISE NOTICE 'Using source table: %', v_src;

  --------------------------------------------------------------------
  -- 0) FIRST TABLE: add "outside peatmap" splits + unspecified allocations
  --------------------------------------------------------------------
  EXECUTE 'DROP TABLE IF EXISTS pa_ghg_reporting.first_ghg_report_2025_applications_with_unespecied';

  EXECUTE format($sql$
    CREATE TABLE pa_ghg_reporting.first_ghg_report_2025_applications_with_unespecied AS
    SELECT
      *,
      ...
      (-1) * "difference" * (
        (COALESCE("%% Actively eroding: Flat bare", 0) +
         ...
         COALESCE("%% Drained: Hagg/gully", 0)) / 100.0
      ...
      (-1) * "difference" * (COALESCE("%% Forested / previously forested", 0) / 100.0)
      ...
      ELSE 0 END AS "Unspecified assigned to Forest",

      -- Unspecified assigned to Modified
      CASE WHEN forestry = FALSE THEN
        (
          (
            (-1) * "difference"
            - (
      ...
      ELSE 0 END AS "Unspecified assigned to Modified",
      ...
      ELSE 0 END AS "Unspecified assigned to Actively Eroding"
  ...
  EXECUTE format($sql$
    ...
    SELECT
      ...
      SUM(COALESCE("Cropland - Arable - Cropland", 0)) AS "Cropland",
      ...
      SUM(COALESCE("Grassland - Intensive grassland - Intensive Grassland", 0)) AS "Intensive Grassland",
      SUM(COALESCE("Grassland - Extensive grassland - Extensive Grassland", 0)) AS "Extensive Grassland",
      SUM(COALESCE("Peat extraction - Industrial - Industrial Extraction", 0)) AS "Industrial Peat Extraction",
      SUM(COALESCE("Peat extraction - Domestic or unknown - Domestic Extraction", 0)) AS "Domestic Extraction",
    ...
    FROM pa_ghg_reporting.first_ghg_report_2025_applications_with_unespecied
    GROUP BY "grant_id"
    ORDER BY "grant_id"
  $sql$, second_tbl, second_tbl);

  -- >>> FIX HERE: drop + create view (no OR REPLACE) <<<
  EXECUTE 'DROP VIEW IF EXISTS pa_ghg_reporting.second_rewetted_summary_table_latest';
  EXECUTE format($v$
    CREATE VIEW pa_ghg_reporting.second_rewetted_summary_table_latest AS
    SELECT * FROM pa_ghg_reporting.%I
  $v$, second_tbl);

  --------------------------------------------------------------------
  -- 2) THIRD TABLE: grant-level summary (ha), timestamped name
  --------------------------------------------------------------------
  EXECUTE format($sql$
    DROP TABLE IF EXISTS pa_ghg_reporting.%I;
    CREATE TABLE pa_ghg_reporting.%I AS
  ...
  -- >>> FIX HERE: drop + create view (no OR REPLACE) <<<
  EXECUTE 'DROP VIEW IF EXISTS pa_ghg_reporting.third_ghg_report_2025_summary_latest';
  EXECUTE format($v$
    CREATE VIEW pa_ghg_reporting.third_ghg_report_2025_summary_latest AS
    SELECT * FROM pa_ghg_reporting.%I
  $v$, third_tbl);

  --------------------------------------------------------------------
  -- 3) EMISSION FACTOR CALCS (APPLICATION naming), timestamped name
  --------------------------------------------------------------------
  EXECUTE format($sql$
    DROP TABLE IF EXISTS pa_ghg_reporting.%I;
    CREATE TABLE pa_ghg_reporting.%I AS
  ...
  -- >>> FIX HERE: drop + create view (no OR REPLACE) <<<
  EXECUTE 'DROP VIEW IF EXISTS pa_ghg_reporting.emission_factors_application_calcs_latest';
  EXECUTE format($v$
    CREATE VIEW pa_ghg_reporting.emission_factors_application_calcs_latest AS
    SELECT * FROM pa_ghg_reporting.%I
  $v$, ef_tbl);
...
FROM pa_ghg_reporting.emission_factors_application_calcs_latest
GROUP BY grant_id
ORDER BY grant_id;

SELECT * FROM pa_ghg_reporting.emission_factors_application_calcs_latest;
